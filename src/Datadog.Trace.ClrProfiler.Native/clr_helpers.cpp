...
RuntimeInformation GetRuntimeInformation(ICorProfilerInfo3* info) {
  ...
  auto hr = info->GetRuntimeInformation(nullptr, &runtime_type, &major_version,
  ...
  return {runtime_type, major_version, minor_version, build_version,
          qfe_version};
}
      ...
      case ELEMENT_TYPE_VAR: {
        ...
        for (i = 0; i < num_generic_arguments; i++) {
          ...
          if (i != type_arg_index) {
            ...
            current_index++;
            ULONG method_arg_index;
            CorSigUncompressData(
                PCCOR_SIGNATURE(&signature[current_index]),
                &method_arg_index);
            size_t j = 0;
            for (auto& generic_param :
                 EnumGenericParams(metadata_import, caller_function_info.id)) {
              if (j++ == method_arg_index) {
                // RETURNING THIS GENERIC PARAM SHOULD WORK BUT IT DOES NOT
                // In MongoDb.Drivers.Core.Server.Servers+ServerChannel.ExecuteProtocol<TResult> we want to cast
                // the returned object of the target method to the GenericParam of the method at index 0.
                // See unbox.any (section III.4.33) https://www.ecma-international.org/publications/files/ECMA-ST/ECMA-335.pdf
                *ret_type_token = generic_param;
                return true;
              }
            }
          }
